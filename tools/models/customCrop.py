...
trainX = np.zeros(shape=(fungusImages+noFungusImages,299, 299, 1), dtype = "float16")
...
for folder in [folderFungus, folderNoFungus]:
    for dirpath, subdirs, files in os.walk(folder):
        for file in files[:]:
            ...
            img = img[105:404,105:404].reshape(299,299,1)
...
def build(input_shape, lr = 1e-4, num_classes= 2, init= 'normal'):
    model = Sequential()
    model.add(Conv2D(64, kernel_size=(3, 3),padding = "valid", strides = 2, input_shape=input_shape,
